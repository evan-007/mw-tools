...
<xsl:key name="recTable" match="recording" use="@n"/>
  ...
  <xsl:template match="/">
    <html>
      <head><title><xsl:value-of select="entry_list/entry/hw"/></title></head>
      
      <body bgcolor="#FFFFFF">
        <h1><xsl:value-of select="entry_list/entry/hw"/></h1>
        <h2><xsl:value-of select="entry_list/entry/def[1]"/>
        </h2>
        <hr/>
